...
const AthleticsScoreWomen100 = () => {
  const [athleticsStandings, setAthleticsStandings] = useState([]);
  ...
  useEffect(() => {
    const getScores = async () => {
      try {
        ...
        const scores = await fetchScores('athletics-women-100');
        
        if (scores.length === 0) {
          setAthleticsStandings([]);
          return;
        }
        ...
        const formattedScores = scores.map(score => ({
          ...
          name: score.name || '',
          score: score.time || score.score || '0' // Try time first, then score field
        }));
        
        // Sort by rank
        const sortedScores = formattedScores.sort((a, b) => a.rank - b.rank);
        setAthleticsStandings(sortedScores);
      } catch (err) {
        console.error('Error fetching athletics scores:', err);
      ...
      <div className="container mx-auto max-w-4xl">
        ...
        >
          <div className="flex items-center bg-[#07534c] px-6 py-3 rounded-full shadow-md">
            ...
            <h1 className="text-2xl font-black text-[#e7fefe]">Athletics Women's 100m Standings</h1>
          </div>
        </motion.header>
        ...
        {/* Standings Table */}
        {!loading && !error && athleticsStandings.length > 0 && (
          <div className="bg-gradient-to-br from-[#07534c] to-[#05433d] rounded-xl shadow-lg overflow-hidden">
            <table className="w-full text-[#e7fefe]">
              ...
              <tbody>
                {athleticsStandings.map((player, index) => (
                  <motion.tr 
                    key={player.id || index} 
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
...
export default AthleticsScoreWomen100;