...
const ChessMatchCard = ({ match, index }) => {
  // Determine the match title
...
const ChessScore = () => {
  ...
  useEffect(() => {
    const getMatches = async () => {
      try {
        setLoading(true);
        // Fetch chess matches from Firebase
        ...
        const formattedMatches = scores.map(score => ({
          id: score.id,
          team1: score.team1 || { 
            name: score.player1 || score.name || 'Player 1', 
            logo: score.logo1 || '/imgs/icon/icon.svg', 
            score: score.score1 || score.score || '0' 
          },
          team2: score.team2 || { 
            name: score.player2 || 'Player 2', 
            logo: score.logo2 || '/imgs/icon/icon.svg', 
            score: score.score2 || '0' 
          },
          date: score.date || 'TBD',
          matchType: score.matchType || 'Chess Match',
          matchNumber: score.matchNumber || '0',
          winner: score.winner || ''
        }));
        
        // Sort matches by match number in descending order
        ...
        const finalMatch = sortedMatches.find(match => match.winner);
        ...
        console.error('Error fetching chess matches:', err);
      ...
      <div className="container mx-auto">
        ...
        >
          <div className="flex items-center bg-[#07534c] px-6 py-3 rounded-full shadow-md">
            ...
            <h1 className="text-2xl font-black text-[#e7fefe]">Chess Championship</h1>
          </div>
        </motion.header>
        ...
        {!loading && !error && matches.length > 0 && (
          <div className="grid gap-4">
            {matches.map((match, index) => (
              <ChessMatchCard key={match.id || index} match={match} index={index} />
            ))}
          </div>
        )}
