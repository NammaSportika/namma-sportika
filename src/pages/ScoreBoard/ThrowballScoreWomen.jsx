...
const ThrowballMatchCard = ({ match, index }) => {
  ...
  const matchTitle = match.matchType 
    ? `Throwball Women's ${match.matchType}` 
    : 'Throwball Women\'s';
  ...
  useEffect(() => {
    const getMatches = async () => {
      try {
        ...
        const scores = await fetchScores('throwball-women');
        ...
        const formattedMatches = scores.map(score => ({
          ...
          matchType: score.matchType || 'Throwball Women',
      ...
      <div className="container mx-auto">
        ...
        >
          <div className="flex items-center bg-[#07534c] px-6 py-3 rounded-full shadow-md">
            ...
            <h1 className="text-2xl font-black text-[#e7fefe]">Throwball Women</h1>
          </div>
        </motion.header>
...
export default ThrowballScoreWomen;