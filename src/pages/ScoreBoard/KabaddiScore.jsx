...
const KabaddiScore = () => {
  ...
  useEffect(() => {
    const getMatches = async () => {
      try {
        setLoading(true);
        // Fetch kabaddi matches from Firebase
        const scores = await fetchScores('kabaddi');
        ...
        const formattedMatches = scores.map(score => ({
          ...
          matchType: score.matchType || 'Kabaddi',
      ...
      <div className="container mx-auto">
        ...
        >
          <div className="flex items-center bg-[#07534c] px-6 py-3 rounded-full shadow-md">
            ...
            <h1 className="text-2xl font-black text-[#e7fefe]">Kabaddi</h1>
          </div>
        </motion.header>
        ...
        {!loading && !error && matches.length > 0 && (
          <div className="grid gap-4">
            {matches.map((match, index) => (
              <KabaddiMatchCard key={match.id || index} match={match} index={index} />
            ))}
          </div>
        )}
